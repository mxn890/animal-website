...
const CustomBanner = ({
  ...
  return (
    <div className={`relative bg-gray-900 text-white overflow-hidden rounded-xl md:rounded-2xl ${className}`}>
      <div className={`container mx-auto px-4 py-8 md:py-16 flex flex-col ${imagePosition === 'right' ? 'md:flex-row' : 'md:flex-row-reverse'}`}>
...
const CatFoodPage = () => {
  const { catFoodProducts } = useProducts();
  const [filteredProducts, setFilteredProducts] = useState(catFoodProducts);
  
  const handleFilterChange = (filters: FilterState) => {
    const filtered = catFoodProducts.filter(product => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-teal-50 to-gray-100">
      {/* Hero Section with Image Background - Made more responsive */}
      <div className="relative h-64 sm:h-80 md:h-96 w-full overflow-hidden">
        ...
        <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
          <div className="container mx-auto px-4 text-center text-white">
            <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold mb-3 sm:mb-4 tracking-tight">
              Premium Cat Nutrition
            </h1>
            <p className="text-base sm:text-lg md:text-xl max-w-2xl mx-auto leading-relaxed">
              Scientifically formulated meals for every stage of your cat's life.
            </p>
            <div className="mt-6 sm:mt-8">
              <button className="bg-white text-teal-700 px-6 py-2 sm:px-8 sm:py-3 rounded-full font-semibold hover:bg-gray-100 transition-all shadow-lg hover:shadow-xl text-sm sm:text-base md:text-lg">
                SHOP NOW!
              </button>
            </div>
          </div>
      ...
      <div className="container mx-auto px-4 sm:px-6 py-6 md:py-8 ">
            ...
            {filteredProducts.length === 0 ? (
              <div className="text-center py-8 md:py-12 bg-white rounded-lg md:rounded-xl shadow-sm text-black">
                ...
                <button 
                  ...
                  className="mt-3 md:mt-4 text-indigo-600 font-medium hover:text-indigo-800 transition-colors text-sm md:text-base"
                >
                  Reset all filters
                </button>
              </div>
        ...
        {/* Value Proposition Section - Responsive layout */}
        <div className="mt-8 md:mt-12 bg-white rounded-lg md:rounded-2xl shadow-sm overflow-hidden">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 divide-y sm:divide-y-0 sm:divide-x divide-gray-100">
            <div className="p-4 md:p-6 text-center">
              <div className="mx-auto w-8 h-8 md:w-10 md:h-10 mb-2 md:mb-3 text-indigo-600">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <p className="text-gray-600 text-xs md:text-sm">Formulated with guidance from feline experts</p>
            </div>
            <div className="p-4 md:p-6 text-center">
              <div className="mx-auto w-8 h-8 md:w-10 md:h-10 mb-2 md:mb-3 text-indigo-600">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <p className="text-gray-600 text-xs md:text-sm">Premium proteins for active cats</p>
            </div>
            <div className="p-4 md:p-6 text-center">
              <div className="mx-auto w-8 h-8 md:w-10 md:h-10 mb-2 md:mb-3 text-indigo-600">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        {/* Promotional Banner - Made responsive */}
        <CustomBanner 
          title="Special Offer for New Customers" 
          subtitle="Get 20% off your first order of premium cat food. Use code CATLOVE at checkout." 
          buttonText="Shop Now" 
